...
IRRepository::IRRepository() : _llvm_context{std::make_shared<llvm::LLVMContext>()} {
  // _add_module(std::string(&jit_aggregate, jit_aggregate_size));
  // _add_module(std::string(&jit_binary_compute, jit_binary_compute_size));
  // _add_module(std::string(&jit_get_table, jit_get_table_size));
  // _add_module(std::string(&jit_table_scan, jit_table_scan_size));
  // _add_module(std::string(&jit_print, jit_print_size));

  // _add_module(std::string(&types, types_size));
  // _add_module(std::string(&value_reader, value_reader_size));
  // _add_module(std::string(&value_writer, value_writer_size));

  _add_module(std::string(&demo, demo_size));
