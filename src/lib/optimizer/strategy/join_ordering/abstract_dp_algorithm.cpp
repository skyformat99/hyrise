...
AbstractDpAlgorithm::AbstractDpAlgorithm(const std::shared_ptr<const JoinGraph>& join_graph):
  _join_graph(join_graph)
{}

std::shared_ptr<JoinPlanNode> AbstractDpAlgorithm::_get_best_plan(const boost::dynamic_bitset<>& vertices) const {
  auto iter = _best_plans.find(vertices);
  DebugAssert(iter != _best_plans.end(), "No plan generated for vertices");
  return iter->second;
}

void AbstractDpAlgorithm::_set_best_plan(const boost::dynamic_bitset<>& vertices, const std::shared_ptr<JoinPlanNode>& plan) {
  _best_plans[vertices] = plan;
}

std::shared_ptr<JoinPlanNode> AbstractDpAlgorithm::_create_join_plan(const std::shared_ptr<JoinPlanNode>& left_plan, const std::shared_ptr<JoinPlanNode>& right_plan) const {
